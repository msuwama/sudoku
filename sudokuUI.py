        ...
        for i in range(9):
            for j in range(9):
                if (i,j) in self.board.initial_cells:
                    e = tk.Label(self.grid_canvas, bg="white",cursor="hand2", text = str(self.board.initial_board[(i,j)]), font = bigfont)
                else:
                    var = tk.StringVar()
                    self.board_vars[(i,j)] = var
                    e = tk.Label(self.grid_canvas, textvariable=var, fg = "blue", bg="white",cursor="hand2", font = bigfont)
